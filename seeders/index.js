...
async function seedUser () {
  try {
    ...
    userToBeInserted = {
      'password':'EJqhqkw4TGuw7LR',
      'username':'Rex_Kassulke',
      'isDeleted':false,
      'isActive':true,
      'userType':authConstant.USER_TYPES.User
    };
    userToBeInserted.password = await  bcrypt.hash(userToBeInserted.password, 8);
    let user = await dbService.findOneAndUpdateDocument(User, { 'username':'Rex_Kassulke' }, userToBeInserted,  {
      upsert: true,
      new: true
    });
    userToBeInserted = {
      'password':'SVtijfEiaesms97',
      'username':'Lukas70',
      'isDeleted':false,
      'isActive':true,
    ...
    userToBeInserted.password = await  bcrypt.hash(userToBeInserted.password, 8);
    let admin = await dbService.findOneAndUpdateDocument(User, { 'username':'Lukas70' }, userToBeInserted,  {
      upsert: true,
      new: true
    });
